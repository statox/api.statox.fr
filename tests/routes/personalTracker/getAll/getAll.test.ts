...
describe('personalTracker/getAll', () => {
    ...
    it('Should verify data isolation: mistakenly attributed data should not be accessible', async () => {
        ...
        await th.mysql.fixture({
            PersonalTracker: [
                ...
                },
                {
                    ...
                    ciphertext: Buffer.from(sodium.from_base64(mistakenlyAttributedEvent.ciphertextB64))
                }
            ]
        });
        ...
        assert.equal(user2Response.body.events.length, 1, 'User2 should only see the mistakenly attributed event');
